...
export const getValidMoves = (curr: Location, board: BoardSquare[][]): Location[] => {
    ...
    if (piece?.isDouble === false) {
        ...
        if (pieceColor === 'white') {
            ...
            } else if (isOnBoard(whiteTL) && board[whiteTL[0]][whiteTL[1]]?.piece === null && board[whiteFL[0]][whiteFL[1]].piece?.color === 'black') {
                validMoves.push(whiteTL)
            }
    
            if (isOnBoard(whiteFR) && board[whiteFR[0]][whiteFR[1]]?.piece === null) {
                validMoves.push(whiteFR);
            } else if (isOnBoard(whiteTR) && board[whiteTR[0]][whiteTR[1]]?.piece === null && board[whiteFR[0]][whiteFR[1]].piece?.color === 'black' && isOnBoard(whiteTR)) {
                validMoves.push(whiteTR)
            }
        } else if (pieceColor === 'black') {
            if (isOnBoard(blackFL) && board[blackFL[0]][blackFL[1]]?.piece === null) {
                validMoves.push(blackFL);
            } else if (isOnBoard(blackTL) && board[blackTL[0]][blackTL[1]]?.piece === null && board[blackFL[0]][blackFL[1]].piece?.color === 'white') {
                validMoves.push(blackTL)
            }
    
            if (isOnBoard(blackFR) && board[blackFR[0]][blackFR[1]]?.piece === null) {
                validMoves.push(blackFR);
            } else if (isOnBoard(blackTR) && board[blackTR[0]][blackTR[1]]?.piece === null && board[blackFR[0]][blackFR[1]].piece?.color === 'white') {
